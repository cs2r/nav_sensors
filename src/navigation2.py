...
class display_sensors(pyglet.window.Window):
    ...
    def on_draw(self):
        ...
        if self.PxWaypoints:
            for pos in self.PxWaypoints:
                self.draw_mark(pos[0], pos[1], 7, (255, 0, 255))
    ...
    def on_mouse_drag(self, x, y, dx, dy, buttons, modifiers):
        if ((self.key == key.P | self.key == key.W) & (self.pxPos[0]!=[])):
            self.pxPos[1] = [x, y]


    def on_mouse_press(self, x, y, button, modifiers):
        if (self.key == key.P) | (self.key == key.W):
            self.pxPos[0] = [x, y]
            if self.key == key.W:
                self.PxWaypoints.append(self.pxPos[0])
...
if __name__ == "__main__":
    ...
    # connect to robot and get the map to local file
    map_name = "bensherif_room"
    try:
        s = sftp.Connection('192.168.0.110', username='waffle', password='g108')
        s.get('maps/' + map_name + '.yaml', "map.yaml")
        with open("map.yaml") as file:
            map_conf = yaml.load(file)
        s.get(map_conf["image"], "map.png")
    except Exception, e:
        print str(e)
    ...
    for name in sensors_name:
        window[name] = display_sensors(name, ranges[i], map_conf)
        pyglet.clock.schedule_interval(window[name].update, 1)
        pyglet.clock.schedule_interval(window[name].waypoint, 1)
        i+=1
    pyglet.app.run()
